...
namespace ZetanStudio.DialogueSystem.Editor
{
    ...
    public abstract class DialoguePort : Port
    {
        ...
        private class NodeEdgeConnectorListener : IEdgeConnectorListener
        {
            ...
            private readonly List<Edge> edgesToDelete;
            ...
            public void OnDrop(GraphView graphView, Edge edge)
            {
                ...
                DialogueGraphNode exit = null;
                var lastExitHere = parent.Target.ExitHere;
                if (lastExitHere) exit = edgesToDelete[0].input.node as DialogueGraphNode;

                graphView.DeleteElements(edgesToDelete);
                edgesToCreate.Clear();
                edgesToCreate.Add(edge);
                List<Edge> edges = graphView.graphViewChanged?.Invoke(graphViewChange).edgesToCreate ?? edgesToCreate;
                edges.ForEach(x => { graphView.AddElement(x); edge.input.Connect(x); edge.output.Connect(x); });

                var child = edge.input.node as DialogueGraphNode;
                if (lastExitHere && (child.Target.Options.Count < 1 || child.Target.Options[0].IsMain && !child.Target.Options[0].Next))
                    child.SetAsExit(exit);
            }
        ...
        public void RefreshIsMain()
        {
            if (!Option.IsMain)
            {
                ...
                if (titleField == null)
                {
                    ...
                    Keyword.Editor.SetAsKeywordsField(titleField);
                }
