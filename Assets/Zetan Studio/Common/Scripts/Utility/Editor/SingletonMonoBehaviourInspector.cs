...
namespace ZetanStudio.Editor
{
    ...
    public class SingletonMonoBehaviourInspector : UnityEditor.Editor
    {
        ...
        private static void OnHierarchyChanged()
        {
            ...
            if (g != null) Debug.LogError(EDL.Tr("存在多个激活的{0}，请确保只激活一个", g.Key.Name));
        }
        ...
        protected bool CheckValid(out string text)
        {
            ...
            {
                System.Text.StringBuilder sb = new System.Text.StringBuilder(EDL.Tr("存在多个激活的<{0}>，请移除或失活其它\n", target.GetType().Name));
                for (int i = 0; i < monos.Length; i++)
                {
                    sb.Append(EDL.Tr("位置 ")); sb.Append(i + 1); sb.Append(": ");
