    ...
    public class ObjectSelectorDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if ((attribute?.type == null || fieldInfo.FieldType.IsAssignableFrom(attribute.type)) && typeof(Object).IsAssignableFrom(type))
            {
                if (attribute != null)
                {
                    Draw(position, property, label, type,
                         () => UtilityZT.Editor.LoadAssets(type, attribute?.resPath, attribute?.extension, attribute?.ignorePackages ?? true),
                         attribute.memberAsName, attribute.memberAsGroup, attribute.memberAsTooltip, attribute.nameNull,
                         attribute.title, attribute.displayNone, attribute.displayAdd);
                }
                else Draw(position, property, label, type,
                          () => UtilityZT.Editor.LoadAssets(type, attribute?.resPath, attribute?.extension, attribute?.ignorePackages ?? true),
                          displayNone: true, displayAdd: true);
            }
            else EditorGUI.PropertyField(position, property, label);
        }

        public static void Draw(Rect position, SerializedProperty property, GUIContent label, Type type,
                                Func<IEnumerable<Object>> objects, string memberAsName = null, string memberAsGroup = null,
        ...
        public static void Draw<T>(Rect position, SerializedProperty property, GUIContent label, Func<IEnumerable<T>> objects,
        ...
        private static AdvancedDropdown GetDropdown<T>(SerializedProperty property, string memberAsName,
                                                       string memberAsGroup, string memberAsTooltip, Func<IEnumerable<T>> objects,
                                                       string title, bool displayNone, bool displayAdd, Type type) where T : Object
        {
            ...
            var dropdown = new AdvancedDropdown<T>(objects?.Invoke(), i => { property.objectReferenceValue = i; property.serializedObject.ApplyModifiedProperties(); },
                                                   i => GetName(memberAsName, i), i => GetGroup(memberAsGroup, i), UtilityZT.Editor.GetIconForObject,
            ...
            (string, Action)[] addCallback()
            {
                List<(string, Action)> callbacks = new List<(string, Action)>();
                {
        ...
        private static void Draw(Type type, Func<AdvancedDropdown> getDropdown, Rect position, SerializedProperty property, GUIContent label, string memberAsName, string memberAsTooltip, string nameNull)
        {
            ...
            if (EditorGUI.DropdownButton(buttonRect,
                                         new GUIContent(name)
                                         {
                                             image = UtilityZT.Editor.GetIconForObject(property.objectReferenceValue),
        ...
        private static void AddCallback(SerializedProperty property, Type type, string assetName)
        {
            var obj = UtilityZT.Editor.SaveFilePanel(() => ScriptableObject.CreateInstance(type), assetName, ping: true);
            if (obj)
            {
        ...
        private static string GetName(string memberAsName, Object obj)
        {
            ...
            {
                if (obj.GetType().GetField(memberAsName, UtilityZT.CommonBindingFlags) is FieldInfo field)
                {
                    name = field.GetValue(obj)?.ToString();
                    break;
                }
                else if (obj.GetType().GetProperty(memberAsName, UtilityZT.CommonBindingFlags) is PropertyInfo property)
                {
                    name = property.GetValue(obj)?.ToString();
                    break;
                }
                else if (obj.GetType().GetMethod(memberAsName, UtilityZT.CommonBindingFlags) is MethodInfo method && method.ReturnType == typeof(string) && method.GetParameters().Where(x => !x.IsOptional).Count() < 1)
                {
        ...
        private static string GetGroup(string memberAsGroup, Object obj)
        {
            ...
            {
                if (obj.GetType().GetField(memberAsGroup, UtilityZT.CommonBindingFlags) is FieldInfo field) group = field.GetValue(obj).ToString();
                else if (obj.GetType().GetProperty(memberAsGroup, UtilityZT.CommonBindingFlags) is PropertyInfo property) group = property.GetValue(obj).ToString();
                else if (obj.GetType().GetMethod(memberAsGroup, UtilityZT.CommonBindingFlags) is MethodInfo method && method.ReturnType == typeof(string) && method.GetParameters().Where(x => !x.IsOptional).Count() < 1)
        ...
        private static string GetTooltip(string memberAsName, string memberAsTooltip, Object obj)
        {
            ...
            {
                if (obj.GetType().GetField(memberAsTooltip, UtilityZT.CommonBindingFlags) is FieldInfo field) tooltip = field.GetValue(obj).ToString();
                else if (obj.GetType().GetProperty(memberAsTooltip, UtilityZT.CommonBindingFlags) is PropertyInfo property) tooltip = property.GetValue(obj).ToString();
                else if (obj.GetType().GetMethod(memberAsTooltip, UtilityZT.CommonBindingFlags) is MethodInfo method && method.ReturnType == typeof(string) && method.GetParameters().Length < 1)
                    tooltip = method.Invoke(obj, null).ToString();
            }
            if (tooltip == name) tooltip = $"{L10n.Tr("Name")}: {name}";
            var path = AssetDatabase.GetAssetPath(obj);
            if (!string.IsNullOrEmpty(path))
                tooltip = $"{L10n.Tr("Path")}: {path}\n{tooltip}";
            return tooltip;
        }
    }
}