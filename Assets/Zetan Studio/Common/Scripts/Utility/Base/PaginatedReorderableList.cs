...
#pragma warning restore IDE1006
        ...
        {
            if (!UtilityZT.Editor.TryGetValue(property, out var value, out var field))
                throw new ArgumentException($"Path '{property.propertyPath}' not exist");

            if (!property.isArray)
                throw new ArgumentException($"Field at path '{property.propertyPath}' is not an array or a list");
            ...
            if (value is null)
            {
                ...
                UtilityZT.Editor.TrySetValue(property, value);
        ...
        private void Refresh()
        {
            try
            {
                ...
                if (isArray)
                {
                    UtilityZT.Editor.TryGetValue(property, out var value);
                    list = value as IList;
                }
        ...
        private void RefreshList()
        {
            ...
            orderList = new ReorderableList(pagedList, eType, m_Draggable, true, m_DisplayAdd, m_DisplayRemove)
            {
                ...
                {
                    rect.size = new Vector2(rect.width, rect.height - 2);
                ...
                drawHeaderCallback = (rect) =>
                {
                    ...
                    style = UtilityZT.Editor.Style.middleRight;
