...
{
    /// <summary>
    /// 一个轻量级的数据类，可以不依赖字段和属性来存储简单的数据<br/>
    /// A field-less utility class for storing simple data.
    /// </summary>
    [Serializable]
    public class GenericData
    {
        ...
        public object this[string key]
        {
            get
            {
                if (TryReadInt(key, out var intValue)) return intValue;
                else if (TryReadBool(key, out var boolValue)) return boolValue;
                else if (TryReadFloat(key, out var floatValue)) return floatValue;
                else if (TryReadString(key, out var stringValue)) return stringValue;
                else if (TryReadData(key, out var dataValue)) return dataValue;
            ...
            set
            {
                ...
                else throw new NotSupportedException($"\"{key}\": {value?.ToString() ?? "Null"}");
            }
        }

        #region 写入列表 Write Into List
        ...
        public T WriteAll<T>(T values) where T : IEnumerable
        {
            ...
            else throw new NotSupportedException(values?.GetType().ToString());
            return values;
        }
        #endregion

        #region 写入字典 Write Into Dictionary
